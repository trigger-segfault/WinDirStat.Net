...
using WinDirStat.Net.Windows.Services.Structures;
			...
			foreach (INtfsNode node in ntfs.EnumerateNodes(state.RootPath)) {
				...
				if (isRoot) {
					...
					if (!state.IsDrive && state.RootPath == node.FullName.ToUpperInvariant()) {
						...
						child = new FolderItem(new WindowsScanFileInfo(node));
					}
				}
				else {
					ExtensionItem extension = Extensions.GetOrAddFromPath(node.Name);
					FileItem file = new FileItem(new WindowsScanFileInfo(node), extension);
					child = file;
					if (!reparsePoint)
